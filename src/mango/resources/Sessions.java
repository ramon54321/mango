...
public class Sessions {
	...
	/**
	 * *
	 * Invalidates the session, this in effect signs out the user. returns: status of signout attempt.
	 */
	@DELETE
	@Produces(MediaType.APPLICATION_JSON)
	...
	/**
	 * *
	 * Creates a session if the given data matches a current user in the database. returns: status of signin attempt.
	 */
	@PUT
	@Produces(MediaType.APPLICATION_JSON)
	...
	public Status sessionsPut(SignIn signIn, @Context HttpServletRequest request) {
		...
		try {
			...
			e.printStackTrace();
			return new Status(false, "Database query failed");
		}
	}
}
